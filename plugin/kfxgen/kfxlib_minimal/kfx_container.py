...
class KfxContainer(YJContainer):
    ...
    def serialize(self):
        ...
        # NOTE: $594/$595 (format_capabilities offset/length) are only added when
        # local_min_id > 595. Template files from Kindle Previewer don't have these
        # fields. Skipping to match template pattern for native generation.
        # Original condition: if self.symtab.local_min_id > 595:
        if format_capabilities is not None and len(format_capabilities_data) > 0:
