...
# locale.getdefaultlocale() is deprecated and removed in Python 3.15.
# getencoding() (3.11+) is the supported replacement for the encoding
# component; fall back to getpreferredencoding for older Calibre Python
# bundles (3.10). Behaviour and the "utf8" guard are preserved. (#105)
if hasattr(locale, "getencoding"):
    LOCALE_ENCODING = locale.getencoding() or "utf8"
else:  # Python < 3.11
    LOCALE_ENCODING = locale.getpreferredencoding(False) or "utf8"
...
def temp_filename(ext, data=None):
    ...
    # Use mkstemp to atomically create the file
    import tempfile as tf

    fd, filename = tf.mkstemp(suffix=ext or "", dir=tempdir())
...
# #47: bound on length-derived allocations in Deserializer.extract. Real KFX
# fragments are rarely >10 MB; 64 MB leaves comfortable headroom while
# rejecting crafted length fields like 0x7FFFFFFF that would otherwise drive
# Calibre's plugin process toward MemoryError or hang. Module attribute so
# tests can monkeypatch a smaller value without allocating the real bound.
_security_log = logging.getLogger(__name__ + ".security")

_DEFAULT_MAX_DECODE_SIZE = 64 * 1024 * 1024
# 1 GB absolute upper bound. The env override (#66) lets ops raise the bound
# for legitimate-large-input cases, but documentation alone is not a
# guardrail — a misconfigured KFXGEN_MAX_DECODE_SIZE=2147483647 would
# silently re-enable the original #47 attack. Clamp at 1 GB so the attack
# surface stays bounded even when the operator is hostile or careless.
_HARD_CEILING_MAX_DECODE_SIZE = 1024 * 1024 * 1024


def _read_max_decode_size_override():
    """Read KFXGEN_MAX_DECODE_SIZE env var if set, else default 64 MB.

    Hard ceiling 1 GB — values above the ceiling are clamped with a
    WARNING and the requested bound is NOT honored. This keeps the #47
    DoS attack surface bounded even when a misconfigured operator sets
    a hostile value. The default 64 MB is comfortable headroom for
    legitimate inputs (real KFX fragments are rarely >10 MB).

    Invalid types, non-positive values, and values exceeding the ceiling
    fall back / clamp to safe values rather than raising.
    """
    raw = os.environ.get("KFXGEN_MAX_DECODE_SIZE")
    if not raw:
        return _DEFAULT_MAX_DECODE_SIZE
    try:
        n = int(raw)
    except (TypeError, ValueError):
        _security_log.warning(
            "ignoring invalid KFXGEN_MAX_DECODE_SIZE=%r (not an integer); using default %d",
            raw,
            _DEFAULT_MAX_DECODE_SIZE,
        )
        return _DEFAULT_MAX_DECODE_SIZE
    if n <= 0:
        _security_log.warning(
            "ignoring non-positive KFXGEN_MAX_DECODE_SIZE=%d; using default %d",
            n,
            _DEFAULT_MAX_DECODE_SIZE,
        )
        return _DEFAULT_MAX_DECODE_SIZE
    if n > _HARD_CEILING_MAX_DECODE_SIZE:
        _security_log.warning(
            "KFXGEN_MAX_DECODE_SIZE=%d exceeds hard ceiling %d; clamping to ceiling",
            n,
            _HARD_CEILING_MAX_DECODE_SIZE,
        )
        return _HARD_CEILING_MAX_DECODE_SIZE
    return n


MAX_DECODE_SIZE = _read_max_decode_size_override()
...
class Deserializer(object):
    ...
    def extract(self, size=None, upto=None, advance=True):
        ...
        # Bound check fires BEFORE the slice so a crafted length field can
        # never even attempt to address gigabytes of memory. (#47)
        if size < 0:
            raise Exception("Deserializer: Negative size (%d)" % size)
        if size > MAX_DECODE_SIZE:
            _security_log.warning(
                "rejected oversized Ion deserialize size %d (max %d)",
                size,
                MAX_DECODE_SIZE,
            )
            raise Exception(
                "Deserializer: size %d exceeds bound %d (security limit)"
                % (size, MAX_DECODE_SIZE)
            )
