...
#: Maximum Ion container nesting depth the binary deserializer will accept.
#: A crafted ~1.4 KB stream of nested list descriptors recurses one Python
#: frame per level; past the interpreter's recursion limit that surfaces as
#: an uncontrolled RecursionError. This bound converts it into a controlled
#: rejection before the limit is reached. Legitimate KFX nests only a handful
#: of levels deep, so 100 is generous headroom while staying well under the
#: interpreter limit. Defense-in-depth: kfxgen is an output plugin and does
#: not normally deserialize untrusted KFX (#123; mirrors the #47 length bound
#: in utilities.Deserializer.extract). Module-level so tests/ops can adjust it.
MAX_ION_NESTING_DEPTH = 100
...
class IonBinary(IonSerial):
    ...
    def deserialize_value(self, serial):
        # Depth-guarded wrapper around the recursive decode. Every container
        # deserializer (list/struct/sexp/annotation) recurses through this
        # single method, so one counter here bounds total nesting and turns a
        # crafted deep-nesting RecursionError into a controlled rejection (#123).
        depth = getattr(self, "_nesting_depth", 0) + 1
        if depth > MAX_ION_NESTING_DEPTH:
            _security_log.warning(
                "Ion nesting depth exceeds security limit (%d); rejecting",
                MAX_ION_NESTING_DEPTH,
            )
            raise Exception(
                "Ion nesting depth exceeds security limit (%d)" % MAX_ION_NESTING_DEPTH
            )
        self._nesting_depth = depth
        try:
            return self._deserialize_value_inner(serial)
        finally:
            self._nesting_depth = depth - 1

    def _deserialize_value_inner(self, serial):
...
def serialize_unsignedint(value):
    """Big-endian minimal-length encoding for arbitrary-precision Python int.

    Replaces the previous `struct.pack(">Q", value)` path (#93), which capped
    at 2**64 - 1 and hard-crashed on larger values. Ion's spec permits
    arbitrary-precision ints; `int.to_bytes` produces no leading zeros, so
    `ltrim0` is no longer needed.
    """
    if value < 0:
        raise Exception("serialize_unsignedint: negative value %d" % value)
    if value == 0:
        return b""
...
def serialize_signedint(value):
    """Sign-magnitude encoding: high bit of the first byte is the sign bit,
    remaining bits are the magnitude in big-endian. Arbitrary-precision (#93).

    A leading 0x00 is prepended when the most-significant magnitude byte has
    its high bit set, so the magnitude isn't mistaken for the sign bit. This
    matches the legacy `ltrim0x` behaviour for the range it could represent
    (|value| < 2**63), while extending support to arbitrary precision above
    that — the old struct.pack path silently lost values >= 2**63.
    """
    if value == 0:
        return b""
    abs_value = abs(value)
    n_bytes = (abs_value.bit_length() + 7) // 8
    data = abs_value.to_bytes(n_bytes, "big")
    if data[0] & 0x80:
        data = b"\x00" + data
    if value < 0:
...
def deserialize_signedint(data):
    ...
    if (data[0] & 0x80) != 0:
        return -int.from_bytes(and_first_byte(data, 0x7F), "big")
...
def deserialize_vluint(serial, max_iterations=16):
    """
    Deserialize a variable-length unsigned integer.

    Args:
        serial: Deserializer object
        max_iterations: Maximum number of bytes to read (default 16, enough for 112-bit values)

    Raises:
        Exception: If data is malformed or exceeds limits
    """
    value = 0
    iterations = 0
    ...
    raise Exception(f"IonVLUInt exceeded maximum iterations ({max_iterations})")
